...
export default function SshConnectionTable({ connections, onDelete }: SshConnectionTableProps) {
  ...
  const columns: Column<Record<string, unknown>>[] = [
    {
      ...
      label: 'Source \u2192 Target',
      render: (row) => {
        const conn = row as unknown as SshConnection;
        return (
    ...
    },
    {
      ...
      render: (row) => {
        const conn = row as unknown as SshConnection;
        return <span className="font-mono text-sm">{conn.ssh_port}</span>;
      },
    },
    {
      key: 'purpose',
  ...
  return (
    <>
      <Table columns={columns} data={connections as unknown as Record<string, unknown>[]} />
      <ConfirmDialog
        open={!!deleteTarget}
